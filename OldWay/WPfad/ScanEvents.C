...
int ScanEvents(TChain* chain, string outname, bool isdata, bool fast, int nEvents, string skimFilePrefix) {
  ...
  TH1F *hETmiss = new TH1F("hETmiss", "Example plot: Missing transverse momentum; p_{T}^{miss} [GeV] ; Events", 50,   0,500);
  TH1F *hMT     = new TH1F("hMT",     "Example plot: Transverse mass; m_{T} [GeV] ; Events ",       100,   0,500);
  hNLeps->SetDirectory(rootdir); hNLeps->Sumw2();
  hLepPt->SetDirectory(rootdir); hLepPt->Sumw2();
  hMT  ->SetDirectory(rootdir); hMT  ->Sumw2();
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	if(index<0) index = ilep;//kannst du denken, was wir hier machen?
	hLepPt->Fill(lep_pt()[ilep]/1000.,eventweight);
      }//ilep
      if(!leptontriggereddata)
	continue;
      hNLeps->Fill(nleps, eventweight);
      hETmiss->Fill(met_et()/1000., eventweight);
      if(nleps==1) {
	//wir haben den Index des Leptons.
	//Da die transverse Massenvariable nicht sehr einfach zu berechnen ist, hbe ich hier die Berechnung aufgeschrieben. Was ich hier mache ist ein LorenzVektor aus den zwei Leptonen bauen, und den Winkel mit der LorentzVector internen Funktion bestimmen.
	//Die Formel dafuer findest du z.B. hier (in Englisch): https://en.wikipedia.org/wiki/Transverse_mass#Transverse_mass_in_two-particle_systems
	//Ein Lorentzvektor ist z.B. hier beschrieben: https://de.wikipedia.org/wiki/Vierervektor
	ROOT::Math::PtEtaPhiEVector lvlep = GetLorentzVector(lep_pt()[index],lep_eta()[index],lep_phi()[index],lep_E()[index]);
	ROOT::Math::PtEtaPhiEVector lvmet = GetLorentzVector(met_et(), 0,met_phi(),met_et());
	float MT = sqrt( pow(lvlep.M(),2)+pow(lvmet.M(),2) + 2*(lvlep.Et()*lvmet.Et() - lvlep.Px()*lvmet.Px() - lvlep.Py()*lvmet.Py() ) ) / 1000.;
